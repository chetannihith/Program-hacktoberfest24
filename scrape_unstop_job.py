...
def calculate_deadline(durations, text, start_date):
    ...
    Args:
        ...
        start_date (str): The start date of the job posting.
...
def scrape_jobs():
    """
    Scrapes job listings from Unstop for different job statuses and saves the data to a combined CSV file.
    """
    ...
    max_jobs_per_status = 10000  # Maximum jobs to scrape per status
    jobs_per_page = 30  # Number of jobs per API call (adjust based on API limits)

    all_jobs = []

    for status in oppstatuses:
        print(f"\nScraping jobs with oppstatus='{status}'")
        search_params = {
            "opportunity": "jobs",
            "oppstatus": status,
            "page": 1,
        ...
        headers["Referer"] = f"https://unstop.com/api/public/opportunity/search-result?opportunity=jobs&oppstatus={status}"

        jobs_scraped = 0  # Counter for jobs scraped per status

        while jobs_scraped < max_jobs_per_status:
            ...
            if data and "data" in data and "data" in data["data"]:
                jobs = data["data"]["data"]
                if not jobs:
                    print(f"No more jobs found for status='{status}' on page {search_params['page']}.")
                ...
                for job in jobs:
                    ...
                    applied = job.get("registerCount", "N/A")
                    impressions = job.get("viewsCount", "N/A")
                    ...
                    regn_requirements = job.get("regnRequirements", {})
                    ...
                    job_entry = {
                        ...
                        "Company": job.get("organisation", {}).get("name", "N/A"),
                        "Location": location_str,
                        "Link": f"https://unstop.com/{job.get('public_url', '')}",
                        "Uploaded On": start_date,
                        "Opportunity Type": job.get("type", "N/A"),
                        "Status": status,  # Track the job's oppstatus
                        "Applied": applied,
                    ...
                    if jobs_scraped >= max_jobs_per_status:
                        break  # Stop if we've reached the maximum number of jobs
            
                print(f"Scraped {len(jobs)} jobs from page {search_params['page']} for status='{status}'. Total jobs scraped for this status: {jobs_scraped}")
        ...
        print(f"Completed scraping {jobs_scraped} jobs for status='{status}'.")

    print(f"\nTotal jobs scraped across all statuses: {len(all_jobs)}")
